...
export const AppProvider = ({children}:{children: ReactNode})=>{
  ...
  const checkAuthOnAppStart = async () => {
    console.log("[Auth] Starting checkAuthOnAppStart...");
    try {
      ...
      if (!storedToken || isTokenExpired(storedToken)) {
        ...
        if (storedRefresh) {
          console.log("[Auth] 4. Tìm thấy RefreshToken, đang thử refresh...");
          await tryRefreshToken(storedRefresh);
        } else {
          console.log("[Auth] 5. Không tìm thấy tokens, xóa auth...");
          await clearAuth();
        }
      } else {
        console.log("[Auth] 6. Token hợp lệ, đang decode...");
      ...
      console.log("[Auth] Finished checkAuthOnAppStart, setting isLoading to false");
      setIsLoading(false);
    }
  };
