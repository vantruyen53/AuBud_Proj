...
import mockBudget from '../../store/seed/budget';
...
import { ModalBudget } from "../../components/customModal";
import { ICategory } from "../../models/IApp";

export default function BudgetScreen() {
  const navigation = useNavigation<HomeStackNavProp>();

  // Quản lý tháng hiện tại đang xem ngân sách
  const [currentMonth, setCurrentMonth] = useState(new Date());
  
  // Trạng thái hiển thị các modal liên quan đến ngân sách
  const [isOpenModalBudget, setIsOpenModalBudget]=useState(false);
  const [isModalVisible, setIsModalVisible] = useState(false)
  
  // Mục tiêu ngân sách (số tiền)
  const [target, setTarget] = useState(0)
  
  // Danh mục được chọn để thiết lập ngân sách
  ...
  // Định dạng hiển thị tháng/năm và khoảng ngày
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          {/* Thẻ hiển thị biểu đồ tròn tổng quan ngân sách */}
          ...
          {/* Danh sách các ngân sách theo danh mục */}
          {mockBudget.map((item)=>{
