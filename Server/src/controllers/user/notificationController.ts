...
export class NotificationController {
  ...
  async getAll(req: Request, res: Response): Promise<void> {
    try {
      ...
      await LogService.write({
        message: `getNotifications error: ${error.message}`,
        actor_type: 'system', type: 'error', status: 'failure',
        actionDetail: 'notification.get.error',
        actorId: (req as any).user?.id, ipAddress: req.ip as string,
  ...
  async markAllAsRead(req: Request, res: Response): Promise<void> {
    try {
      ...
      await LogService.write({
        message: `markAllAsRead error: ${error.message}`,
        actor_type: 'system', type: 'error', status: 'failure',
        actionDetail: 'notification.mark_read.error',
        actorId: (req as any).user?.id, ipAddress: req.ip as string,
        metaData: { error: error.message, stack: error.stack } as any,
      });
      res.status(500).json({ success: false, message: error.message });
